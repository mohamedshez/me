...
- Every tested production artifact gets a clean semantic tag such as `v1.0.1`. Before building, CI reads all GitHub tags and selects the next unused patch version, using `package.json` as the minimum version. Older build-suffix tags remain unchanged as historical records.
...
- Tags are created **after tests pass and before deployment**, so failed tests never produce a release tag. A deployment failure may leave a valid tested-build tag; the tag alone does not claim the build went live. If a run already created its tag, a retry allocates the next unused patch version. GitHub atomically rejects duplicate tag creation; old tags are never overwritten. A failure before tagging may reuse the unclaimed version.
- `package.json` sets the minimum release version and can request a major/minor/patch bump. CI automatically advances the patch for every subsequent deployment, even for the same commit, without version-bump commit loops. The full commit remains available in `/api/version`, and the footer links to its exact release tag.
...
The original portrait in `public/images/mohamed-shez.jpg` is sourced from the owner's public GitHub avatar. The website uses the conservatively enhanced `public/images/mohamed-shez-enhanced.png`; the original is retained. Approved CV skills, education, courses, and interests are curated in `src/content/portfolio.ts` rather than automatically ingesting employment profiles. The homepage, About page, and contact section link to `public/cv/Mohamed-Shez-CV.pdf`, a public skills-and-projects CV prepared from the supplied CVs. The original CVs are not published: the public edition preserves the personal-only content policy. Update the curated content and public PDF together when skills or education change.
